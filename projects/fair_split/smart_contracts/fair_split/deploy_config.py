...
def deploy() -> None:
    ...
    # Platform address - using deployer for testnet
    platform_address = deployer.address
    logger.info(f"Platform Fee Address: {platform_address}")
    logger.info("Creating application...")

    try:
        # Get artifacts path
        artifacts_path = Path(__file__).parent.parent / "artifacts" / "fair_split"
        ...
        logger.info(f"Schema: Global({global_uints}u, {global_bytes}b), Local({local_uints}u, {local_bytes}b)")
        
        # Calculate method selector from signature
        from algosdk.abi import Method
        from algosdk import encoding
        
        method_obj = Method.from_signature("create_contract(address)string")
        method_selector = method_obj.get_selector()
        logger.info(f"Using method selector: {method_selector.hex()}")
        
        # Encode address argument
        platform_addr_bytes = encoding.decode_address(platform_address)
        
        # Combine method selector + encoded args
        app_args = [
            method_selector,
            platform_addr_bytes,
        ]
